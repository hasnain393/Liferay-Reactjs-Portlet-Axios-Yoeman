...
public class DefaultController implements MVCRenderCommand{
	...
	public String render(RenderRequest renderRequest, RenderResponse renderResponse) throws PortletException {
		...
		SearchContainer<Student> searchContainer =
				new SearchContainer<Student>
		(renderRequest, null, null, "view_employee_detail", 
				SearchContainer.DEFAULT_DELTA, iteratorURL, null, StringPool.BLANK);
		...
		System.out.println("Test"+ studentLocalService.getStudentsCount());
		 ...
		 List<Student> users = studentLocalService.getStudents(-1, -1);
		 System.out.println("-1 -1"+users);
		 System.out.println(users.getClass().getName().toString());
		 System.out.println(users.getClass().getTypeName().toString());
